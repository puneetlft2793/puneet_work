...
namespace UsbApis
{
	...
	public:
		__declspec(dllexport) HRESULT OpenDevice();					//to open the device
		__declspec(dllexport) HRESULT CloseDevice();					//to close the usb device
		//Sd card read write functoins
		__declspec(dllexport) HRESULT RdFileFromSdCard0(
		...
		__declspec(dllexport) HRESULT WrFileToSdCard0(
								DeviceTypes sppuUsbDeviceTypes,
		...
		__declspec(dllexport) HRESULT RdMemFromSdCard0(
								DeviceTypes sppuUsbDeviceTypes,
								char *fname = NULL, 
								DWORD addr = DEFAULT_SD_READ_ADDR, 
								DWORD numBytes = DEFAULT_NUM_OF_BYTES,
		...
		__declspec(dllexport) HRESULT WrMemToSdCard0(
								DeviceTypes sppuUsbDeviceTypes,
								char *fname = NULL, 
								DWORD addr = DEFAULT_SD_WRITE_ADDR, 
								DWORD numBytes = DEFAULT_NUM_OF_BYTES, 
		...
		__declspec(dllexport) HRESULT UpdateFirmWare(
								DeviceTypes Dev,
								char *fname);

		__declspec(dllexport) HRESULT RegisterRead(
								UINT32 addr,
								UINT32 *val);

		__declspec(dllexport) HRESULT RegisterWrite(
								UINT32 addr,
								UINT32 val);

		__declspec(dllexport) HRESULT RegisterRead_V2(
								UINT32 addr,
								UINT32 *val);

		__declspec(dllexport) HRESULT RegisterWrite_V2(
								UINT32 addr,
								UINT32 val);
		...
		__declspec(dllexport) HRESULT Testing(void *);
