...
int MiddleAPI::updatedac_default(SPPUHostConnectionType a_enConnectionType, unsigned int a_nModuleID)
{
	char strDebug[DEBUG_STR_LEN]	=	{'\0'};
	unsigned int dac_default[98] = 
	{
		0x0000, 0x019C,
		0X0001, 0x100E,
		0x0002, 0xF080,
	...
	//Read DacRegCongic.txt file and parse address and value pair. (for 1 register only)
#pragma region FileParse

	//1. Open and Read File till file end (Each Reg takes 13 bytes of info)
		FILE *rdBinFile;
		FILE *writeoutFile;
		int nlines = 0;
		/* opening the file */
		if(!(rdBinFile = fopen("D:\\SPPULRDE\\SPPU_LRDE\\release\\ConfigDacRegs.txt", "r+")))
		{
			mPrintDebug("******************************************************FILE didnt open for read*****************************************************************");
			return -1;
		}
			mPrintDebug("******************************************************FILE open for read*****************************************************************");
		if(!(writeoutFile = fopen("c:\\temp2\\copyConfigDacRegs.txt", "a+")))
		{
			mPrintDebug("******************************************************FILE didnt open for write*****************************************************************");
			return -1;
		}
		mPrintDebug("******************************************************FILE open for write*****************************************************************");
		/* Get file size */
		fseek(rdBinFile, 0, SEEK_END);
		DWORD dwFileSize = ftell(rdBinFile);
		fseek(rdBinFile, 0, SEEK_SET);
		char buffer[98];
		if((fgets(buffer,98,rdBinFile) != NULL))
		{ 
		mPrintDebug("******************************************************save into char buffer*****************************************************************");
		}
		else
		mPrintDebug("******************************************************didnt save into char buffer*****************************************************************");
		
		char *point ;
		/*int j = 0;


		unsigned int dac_default1[10];
		for(j=0;j<10;j++)
		{
		fscanf(rdBinFile, "%u", &dac_default1[j]);
		
		}*/
	//2. Parse for one reg (you should get Address and Value pair in <int> variables)
		//mPrintDebug("******************************************************Before while loop*****************************************************************");
		/*while((fgets(buffer,1024,rdBinFile) != NULL))
		{ 
		mPrintDebug("******************************************************come into while loop*****************************************************************");
		nlines++; //count number of input lines
		fputs(buffer,writeoutFile);
		}
		unsigned int k = 0;
		char buffer2[1024];
		for(k=0;k<1024;k++)
		{
			...
			//fputs(buffer2,writeoutFile);*/
		//	mPrintDebug("******************************************************after while loop*****************************************************************");
	//3. Close the file after getting its data size
		fclose(rdBinFile);
	
	//4. PArse the old array 

	//5. Update the new value.
...
#pragma endregion FileParse

	int i = 0;

	for(i=0 ; i<98;i+=2)
	{
		RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_DAC1, dac_default[i], dac_default[i+1]);
	}

	/*for(i=0 ; i<10;i+=2)
	{
		RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_DAC1, dac_default1[i], dac_default1[i+1]);
	}*/
...
void MiddleAPI::DefaultInDACRegisters(SPPUHostConnectionType a_enConnectionType,unsigned int a_nModuleID,SPPUDACType enDactype )
{
	...
	case LFT_SPPU_DAC_1:
		...
		mPrintDebug("********************************************DEFAULT SETTINGS from Arrey********************************************************");
		updatedac_default(a_enConnectionType, a_nModuleID);
			...
			case LFT_SPPU_FPGA_PROP_AMPLITUDE_RF_channel_2:
				...
				else if(rdval_rf2 == 0x02)
				{
					...
					switch((SPPUFPGAPropAmplitude)a_nPropValue)
					{
						...
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x7B);
						break;
						case LFT_SPPU_FPGA_PROP_MINUS_23:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x76);
						break;
						case LFT_SPPU_FPGA_PROP_MINUS_22:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x72);
						break;
						case LFT_SPPU_FPGA_PROP_MINUS_21:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x6F);
						break;
						case LFT_SPPU_FPGA_PROP_MINUS_20:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x69);
						break;
						case LFT_SPPU_FPGA_PROP_MINUS_19:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x66);
						break;
						case LFT_SPPU_FPGA_PROP_MINUS_18:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x62);
						break;
						case LFT_SPPU_FPGA_PROP_MINUS_17:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x5E);
						break;
						case LFT_SPPU_FPGA_PROP_MINUS_16:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x5A);
						break;
						case LFT_SPPU_FPGA_PROP_MINUS_15:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x56);
						break;
						case LFT_SPPU_FPGA_PROP_MINUS_14:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x52);
						break;
						case LFT_SPPU_FPGA_PROP_MINUS_13:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x4E);
						break;
						case LFT_SPPU_FPGA_PROP_MINUS_12:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x4A);
						break;
						case LFT_SPPU_FPGA_PROP_MINUS_11:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x46);
						break;
						case LFT_SPPU_FPGA_PROP_MINUS_10:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x42);
						break;
						case LFT_SPPU_FPGA_PROP_MINUS_9:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x3E);
						break;
						case LFT_SPPU_FPGA_PROP_MINUS_8:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x39);
						break;
						case LFT_SPPU_FPGA_PROP_MINUS_7:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x36);
						break;
						case LFT_SPPU_FPGA_PROP_MINUS_6:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x31);
						break;
						case LFT_SPPU_FPGA_PROP_MINUS_5:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x2E);
						break;
						case LFT_SPPU_FPGA_PROP_MINUS_4:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x2A);
						break;
						case LFT_SPPU_FPGA_PROP_MINUS_3:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x26);
						break;
						case LFT_SPPU_FPGA_PROP_MINUS_2:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x22);
						break;
						case LFT_SPPU_FPGA_PROP_MINUS_1:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x1E);
						break;
						case LFT_SPPU_FPGA_PROP_0:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x19);
						break;
						case LFT_SPPU_FPGA_PROP_1:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x15);
						break;
						case LFT_SPPU_FPGA_PROP_2:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x11);
						break;
						case LFT_SPPU_FPGA_PROP_3:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x0E);
						break;
						case LFT_SPPU_FPGA_PROP_4:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x09);
						break;
						case LFT_SPPU_FPGA_PROP_5:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x06);
						break;
						case LFT_SPPU_FPGA_PROP_6:
						RegWrite(a_enConnectionType, a_nModuleID, LFT_SPPU_DEV_ATTENUATOR, 1, 0x01);
						break;
					}
